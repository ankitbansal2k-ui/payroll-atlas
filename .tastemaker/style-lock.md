...
- **Motion:** GSAP via CDN (`cdnjs`) for tweens. Reveal triggering uses `IntersectionObserver`, not ScrollTrigger — a live-site check after the first deploy found ScrollTrigger's `scrollTrigger: {start, once}` shorthand on `gsap.fromTo` leaving above-the-fold content stuck at its pre-animation (blurred/opacity 0) state on GitHub Pages, a direct violation of "show the page at rest." Rebuilt on IntersectionObserver (which fires reliably for already-in-view elements) plus a 2s hard safety timeout that forces full visibility no matter what. One exception: the tagline word-reveal needs continuous scroll-progress (not a binary trigger), so it uses a `requestAnimationFrame`-throttled, passive `scroll` listener — a deliberate, documented departure from the "never use `window.addEventListener('scroll')`" rule, scoped to the one effect that genuinely needs continuous progress rather than a threshold trigger.
